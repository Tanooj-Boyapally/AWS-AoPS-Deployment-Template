...
provider "aws" {
  region     = var.region
  access_key = var.aws_access_key    # Optional: use environment variables instead
  secret_key = var.aws_secret_key    # Optional: use environment variables instead
}
...
# Get current caller identity for AWS account ID
...
# EKS Cluster with restricted access
resource "aws_eks_cluster" "eks_cluster" {
  ...
  vpc_config {
    ...
    # Add your current public IP here (replace X.X.X.X with your actual IP)
    public_access_cidrs     = ["103.48.91.0/24"]
  }
